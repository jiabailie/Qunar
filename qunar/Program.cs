...
namespace qunar
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            source.Save("E:/Projects/qunar-file/test/2_do_image_uniformization.bmp", ImageFormat.Bmp);

            Operations.Generate_White_Edges(source);

            Operations.Fill_One_Width_Blanks(source);

            source.Save("E:/Projects/qunar-file/test/3_remove_edges.bmp", ImageFormat.Bmp);

            source = Scaling.Image_Zoom_Out(2, source);

            source.Save("E:/Projects/qunar-file/test/4_image_zoom_out.bmp", ImageFormat.Bmp);
#else
            if (args.Length > 0)
            {
